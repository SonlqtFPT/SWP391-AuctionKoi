...
function EnterPrice({
  ...
  const handleBid = async () => {
    try {
      const response = await axios.post(post_bid_api, {
        lotId: lotId,
        price: bidPrice.replace(/\./g, ""), // Chuyển đổi giá trị input về dạng số, /\./g là biểu thức regular expression, g là gobal tức là tất cả
        memberId: currentMemberId,
      });
      if (response.data.message === "Member is not registered for the Lot") {
        // Gửi yêu cầu đăng ký
        const regResponse = await axios.post(post_regis_api, {
          memberId: currentMemberId,
          lotId: lotId,
        });
        const link = regResponse.data; // Giả sử link được trả về trong regResponse.data.link
        console.log("Link nè: ", link);
        setRegistrationLink(link); // Cập nhật link đăng ký
      } else {
        toast.success(response.data.message);
        fetchLot();
        setRegistrationLink(""); // Đặt lại link nếu không có thông báo lỗi
      }
    } catch (error) {
      console.error("Error submitting bid:", error);
      console.log("link nek: ", registrationLink);
    }
  };
