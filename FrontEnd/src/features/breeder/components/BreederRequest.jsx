  ...
  const columns = [
    ...
    },
    {
      ...
      render: (requestedAt) => {
        ...
        const formattedDate = date.toLocaleDateString("en-GB"); // Định dạng ngày theo kiểu
    ...
    },
    {
      ...
      key: "status",
      render: (status) => {
        // Chỉnh sửa giá trị status theo yêu cầu
        switch (status) {
          case "PENDING":
          ...
          case "PENDING_NEGOTIATION":
            return <span>Pending negotiation</span>;
          case "APPROVE":
            return <span>Approve</span>;
          case "INSPECTION_PASSED":
            return <span>Inspection passed</span>;
          default:
            return <span>{status || "PENDING"}</span>; // Hiển thị "Pending" nếu status rỗng
        }
      },
    },
    {
    ...
    },
    {
      ...
      key: "message",
      render: (text, fish) => {
        const { offerPrice, offerAuctionType } = fish; // Giả sử bạn đã lấy offerPrice và offerAuctionType từ API
        if (offerPrice > 0 || offerAuctionType !== fish.auctionTypeName) {
          return (
            <div>
              {offerPrice > 0 && <div>Offer Price: {offerPrice}</div>}
              {offerAuctionType && (
                <div>Offer Auction Type: {offerAuctionType}</div>
              )}
            </div>
          );
        }
        return null; // Không hiển thị gì nếu không có điều kiện nào thỏa mãn
      },
    },
  ];
